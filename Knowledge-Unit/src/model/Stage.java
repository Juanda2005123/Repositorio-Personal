...
public class Stage {
     ...
     * @return boolean
     */
    public boolean registerKnowledgeUnit(String idKU, String descriptionKU, TypeUnits typeKU, String collaboratorName, String collaboratorPosition, String learnedLessonsKU) {

        KnowledgeUnit newKnowledgeUnit = new KnowledgeUnit(idKU, descriptionKU, typeKU, collaboratorName, collaboratorPosition, learnedLessonsKU);
    ...
    public int[] informTypeUnits() {
		...
		for(int i = 0; i < units.length; i++) {
            if(units[i]!=null){
                ...
                if(typeKU==TypeUnits.TECHNICAL) {
                informArray[0] += 1;
                } else if(typeKU==TypeUnits.EXPERIENCES) {
                informArray[1] += 1;
                } else if(typeKU==TypeUnits.MANAGEMENT) {
                informArray[2] += 1;
                } else if(typeKU==TypeUnits.DOMAIN) {
                informArray[3] += 1;
                }
            }
		}
    ...
    public boolean informCollaboratorUnits(String collaboratorName) {
        ...
        for(int i = 0; i < units.length; i++) {
            if(units[i]!=null) {
                ...
                if(collaboratorName.equals(realCollaboratorName)){
                flag = true;            
                }
            }
		}
