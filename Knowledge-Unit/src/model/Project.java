...
public class Project{
	 ...
	 */
	public Project(String projectName, String clientName, double budget, Calendar calendarStart, Calendar start_up_stage_calendar, Calendar analysis_stage_calendar, Calendar design_stage_calendar, Calendar execution_stage_calendar, Calendar closing_stage_calendar, Calendar follow_up_stage_calendar){
	 ...
	 * @return boolean
	 */
	public boolean registerKnowledgeUnit(String stagePhase, String idKU, String descriptionKU, TypeUnits typeKU, String collaboratorName, String collaboratorPosition, String learnedLessonsKU) {

		for(int i = 0; i < stages.length; i++) {
			if(stages[i]!=null){
				String realStagePhase = stages[i].getStagePhaseString();
				if(realStagePhase.equalsIgnoreCase(stagePhase)) {
					if(stages[i].registerKnowledgeUnit(idKU, descriptionKU, typeKU, collaboratorName, collaboratorPosition, learnedLessonsKU)){
						return true;
					}
				}
			}
	...
	public int[] informTypeUnits() {
		...
		for(int i = 0; i < stages.length; i++) {
			if(stages[i]!=null) {
				informArray = stages[i].informTypeUnits();
	 ...
	 * @return boolean
	 */
	public boolean informCollaboratorUnits(String collaboratorName) {

		boolean flag = false;
		for (int i = 0; i < stages.length; i++) {
			if(stages[i]!=null) {
				flag = stages[i].informCollaboratorUnits(collaboratorName);
				if(flag==true){
					return flag;
				}
			}
